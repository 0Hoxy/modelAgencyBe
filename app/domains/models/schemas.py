...
from .model_enum import (GenderEnum, VisaTypeEnum, CameraTestStatusEnum)
...
# --- 카메라테스트 스키마 ---
class CreateCameraTest(BaseModel):
    model_id: uuid.UUID = Field(..., description="모델 ID")
    is_tested: CameraTestStatusEnum = Field(default=CameraTestStatusEnum.PENDING, description="카메라테스트 상태")
    visited_at: datetime | None = Field(default=None, description="방문일시")

class UpdateCameraTestStatus(BaseModel):
    is_tested: CameraTestStatusEnum | None = Field(default=None, description="카메라테스트 상태")
    visited_at: datetime | None = Field(default=None, description="방문일시")

class ReadCameraTest(BaseModel, OrmConfigMixin):
    id: int = Field(..., description="카메라테스트 ID")
    model_id: uuid.UUID = Field(..., description="모델 ID")
    is_tested: CameraTestStatusEnum = Field(..., description="카메라테스트 상태")
    visited_at: datetime | None = Field(default=None, description="방문일시")
